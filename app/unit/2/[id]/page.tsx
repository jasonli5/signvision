...
import unitData from "@/public/data/unit_2.json";
...
export default function Page({ params }: { params: { id: string } }) {
  ...
  const [model, setModel] = useState<string>(unit.model);
  ...
  return (
    <>
      <div className="flex-col w-auto justify-center items-center m-auto py-5 bg-slate-100 rounded-lg shadow-md inline-flex relative">
        <Link href="/unit/2" className="absolute top-4 left-4">
        ...
        {contentType === "youtube" && (
          <YouTubeCard
            character={content.character!}
            youtubeLink={content.description!}
            continueOnClick={onContinue}
          />
        )}
        <div
...
const YouTubeCard = ({
  ...
  return (
    <>
      ...
      <ContinueButton onClick={continueOnClick} />
    </>
  );
};
